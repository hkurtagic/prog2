...
public class WatchlistViewController implements Initializable {
    ...
    public List<Movie> allMovies = new ArrayList<>(watchlistRepository.toMovies(watchlistRepository.getAll()));
    ...
    public WatchlistViewController() throws SQLException {
    }
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        movieListView.setCellFactory(movieListView -> new WatchlistMovieCell()); // use custom cell factory to display data
    ...
    public static List<Movie> getMoviesBetweenYears(List<Movie> movies, int startYear, int endYear) {
        return movies.stream().filter((Movie movie) -> movie.getReleaseYear() <= endYear && movie.getReleaseYear() >= startYear).toList();
    }

    public static int getLongestMovieTitle(List<Movie> movies) {
        return movies.stream().map(Movie::getTitle).mapToInt(String::length) // Map each title to its length
                .max() // find max length
                .orElse(0); // return 0 if movie list is empty
    }

    public static long countMoviesFrom(List<Movie> movies, String directors) {
        return movies.stream().filter((Movie movie) -> movie.getDirectors().contains(directors)) // Filter movies by
                // director
